...
<html>
	<head>
	    ...
	    -->
	    <?php
	    	$handle = fopen("system/kernel/database/system/apps_installed.json","rt");
			$data = fread($handle, 10000000);
			$res = json_decode($data);
			fclose($handle);
			for($i=0;$i<count($res);$i++){
				echo '<script src="system/applications/'.$res[$i].'/Run.js"></script>';
			}
	    ?>
	    <script language="JavaScript">
			...
			var appInstall;
			if(!isEmulator){
				myApps = <?php echo $data; ?>;
				appInstall = <?php echo count($res); ?>;
				Startup();
			}
		</script>
	</head>
