...
function installer_install(dir, fileName, isReplace){
    $.post("system/applications/installer/script/CopyApps.php?filename="+fileName+"&destdir=../../&replace="+isReplace).done(function(data){
        if(data == "SUCCESS"){
            ...
            MessageBox.show("Installer", "Application "+fileName.replace(".dapp","")+" already installed, want you reinstall this application?", null, "OK_CANCEL", "installer_install('"+dir+"','"+fileName+"','Y')", "CloseInstaller()", "CloseInstaller()");
        }
    });
