function loadDirectory(){
	...
	$.post("system/applications/runnable/extract/file_explorer/script/directory-explorer.php", {"dir" : "~"}, function(data){
		res = eval("("+data+")");
		...
		for(i=0;i<res.sum;i++){
			if(res.isDir[i]){
                ...
                if(res.dirInFolder[i] > 0)
                    imgDraftMore = "<img src='system/applications/runnable/extract/file_explorer/images/trans.png' id='"+thisId+"img' class='nav-dir' onclick=moreDraft('"+res.thisDir+""+thisValue+"/','dir_"+thisId+"','"+thisId+"img') />";
                else
                ...
                $("#body-explorer-left").append(
					...
					"<img src='system/applications/runnable/extract/file_explorer/images/directory.png' id='"+thisId+"img' class='directory' /> " + res.value[i] + "</div>" +
...
function showDraft(dir, backTrack, nextTrack, textSearch){
    ...
    $.post("system/applications/runnable/extract/file_explorer/script/directory-explorer.php", {"dir" : dir+"//"}, function(data){
		res = eval("("+data+")");
        DirResult = res;
		
		i = 0;
		$("#body-explorer-right").html("");
		$.post("system/applications/runnable/extract/file_explorer/script/get-filetype.php", {"filetype" : res.fileType})
		.done(function(data){
			//alert(data);
            if(data != null && data != ""){
                result = eval("("+data+")");
			...
			function showThis(){
				...
				if(res.isDir[i]){
					if(textSearch == "" || thisValue.toLowerCase().replace(/_-_-_/g," ").search(textSearch.toLowerCase()) > -1){
						...
						"<img src='system/applications/runnable/extract/file_explorer/images/folder.png' class='draft-dir-img'/>" +
					...
					if(textSearch == "" || thisValue.toLowerCase().replace(/_-_-_/g," ").search(textSearch.toLowerCase()) > -1){
						...
						if(result[res.fileType[i]] == "" || result[res.fileType[i]] == undefined){
							icon = "system/applications/runnable/extract/file_explorer/images/file-type/unknown-type.png";
						} else {
...
function moreDraft(dir, id, idImg){
	...
	if($("#"+id).html() == "" || $("#"+id).html() == null){
		...
		$("#"+idImg).css({
			background : "url(system/applications/runnable/extract/file_explorer/images/dir-open.png)"
		})
		.mouseover(function(){
			$(this).css({
				background : "url(system/applications/runnable/extract/file_explorer/images/dir-hover-open.png)"
			})
		})
		.mouseout(function(){
			$(this).css({
        ...
        $.post("system/applications/runnable/extract/file_explorer/script/directory-explorer.php", {"dir" : dir+"//"}, function(data){
			...
			for(i=0;i<res.sum;i++){
                ...
                if(res.dirInFolder[i] > 0)
                    imgDraftMore = "<img src='system/applications/runnable/extract/file_explorer/images/trans.png' id='"+thisId+"img' class='nav-dir' onclick=moreDraft('"+thisDir+""+thisValue+"/','dir_"+thisId+"','"+thisId+"img') />";
                else
                    imgDraftMore = "<img style='display: inline-block;height: 12px;width:12px'/>";
                if(res.isDir[i]){
					$("#"+id).html($("#"+id).html() + 
					"<div class='dir' id='"+thisId+"dir' onclick=dirLeftSelected('"+thisId+"dir') onmousedown=showDraft('"+thisDir+thisValue+"/')>" + imgDraftMore+
					"<img src='system/applications/runnable/extract/file_explorer/images/directory.png' id='"+thisId+"img' class='' /> " +
					res.value[i] + "</div>" +
		...
		$("#"+id).html("");
		$("#"+idImg).css({
			background : "url(system/applications/runnable/extract/file_explorer/images/dir-close.png)"
		})
		.mouseover(function(){
			$(this).css({
				background : "url(system/applications/runnable/extract/file_explorer/images/dir-hover.png)"
			})
		})
		.mouseout(function(){
			$(this).css({
