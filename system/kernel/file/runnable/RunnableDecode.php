	...
	for($i=0;$i<$len;$i++){
		if($data[$i] == "["){
			...
			if($val == "[ENDFILE]"){
				...
				$program_id = str_replace(".dapp","",$filename);
				$file = str_replace("Minimizer.close(\"".$program_id."\")","Minimizer.close(\"".$program_id."\");Runnable.close(\"".$program_id."\")",$file);
				$file = str_replace("/applications/","/applications/runnable/extract/",$file);
				$file = str_replace("\/applications\/","\/applications\/runnable\/extract\/",$file);
